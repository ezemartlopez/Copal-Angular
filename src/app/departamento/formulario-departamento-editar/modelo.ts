export class Campo {
  ...
  export class Formulario {
    ...
    obtenerObjeto(){
      return JSON.parse(this.obtenerRespuestaJSON());
    }
...
export function validarEmail(email: string): string{
  ...
  export function usuarioValido(valor: string): string {
    if (valor === '0') {
      return 'Debe elegir un Usuario Valido';
    }
    return '';
  }
  export function rolValido(valor: string): string {
    if (valor === '1') {
      return 'Debe elegir un Rol Valido.';
    }
    return '';
  }